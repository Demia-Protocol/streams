    ...
    T: for<'a> Transport<'a, Msg = TransportMessage, SendResponse = TSR> + Send,
{
    ...
    {
        let prepared = self
            .prepare_signed_packet(topic, public_payload, masked_payload)
            .await?;
...
{
    pub async fn prepare_signed_packet<P, M, Top>(
        &mut self,
        topic: Top,
        public_payload: P,
        masked_payload: M,
        ...
        Top: Into<Topic>,
    {
        ...
        permission = self
            .check_and_update_permission(MessageType::SignedPacket.into(), &topic, permission, None)
            .await?
            .1;
